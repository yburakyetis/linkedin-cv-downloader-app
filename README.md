...
- ⏯️ **Resume Capability**: Continue downloading from a specific page or pick up where you left off
- 🔐 **Session Persistence**: Saves your LinkedIn login session to avoid repeated logins
- 🛡️ **Self-Healing Recovery**: Automatically handles and recovers from "Virus Scan" reloads and network interruptions
- 🎭 **Stealth Mode**: Uses advanced anti-bot detection techniques to mimic human behavior
- 🛑 **Graceful Stop**: Safely stop the process at any time without crashing or losing session data
...
2.  **Initial Login**
    ...
    - Wait for the login to complete (the application will automatically detect when you've logged in)
    - If you have 2FA enabled, complete it in the browser window
...
-   **Interruptions**: If you need to stop the process, click the **"Stop"** button in the dashboard. This will gracefully close the browser and save your progress.
-   **Virus Scans**: LinkedIn occasionally reloads pages for "Virus Scans". The application is designed to detect this and automatically recover your position.
...
│   │   └── state-manager.js # State persistence service
...
1. **Session Management**: Uses `StateManager` and Playwright's persistent context to maintain browser sessions and download progress across runs
2. **Stealth Mode**: Implements `playwright-extra-plugin-stealth` and randomized human-like delays to avoid bot detection
3. **Self-Healing**: Automatically reloads and recovers position if interrupted by LinkedIn's security checks or network issues
4. **Natural Interactions**: 
   - Randomized delays between actions
   - Natural scrolling and mouse behavior
5. **Pagination**: Automatically detects and navigates through multiple pages of applicants
6. **Error Handling**: Gracefully handles missing CVs and continues processing
